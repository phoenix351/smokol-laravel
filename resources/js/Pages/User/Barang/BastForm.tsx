...
const BastForm = ({ open, form, onCancel, user }: { open: boolean, form: FormInstance, onCancel: () => void, user: User }) => {
    ...
    const fetchBarangs = async (user_id: string | number) => {
        try {
            ...
            const { data } = await axios.get(`/api/barang?isUser=1&userId=${user_id}`);
            console.log({ x:data.data[0] });
            const convertedBarangs = data.data.map((item: any) => ({ label: `${item.barang.merk}/${item.barang.tipe}`, value: item.id }))
    ...
    const fetchUsers = async () => {
        try {
            ...
            // console.log({ data });
            const convertedItems = data.map((item: any) => ({ label: item.nama_lengkap, value: item.id }))
    ...
    const onFinish = async (values: FormData) => {
        try {
            ...
            // const { data } = await axios.get(`/api/barang/user/${user_id}`);
            // console.log({ data });
    ...
    return (
        <Modal open={open} confirmLoading={confirmLoading} onCancel={onCancel} title="Formulir Pembuataan BAST">
            ...
            <Form onFinish={onFinish} form={form}>
                ...
                <Form.Item label="Pihak I (Pemberi BMN)" name='pihak1_user_id'>
                    <Select options={users} disabled={currentRole === 1} />
                </Form.Item>
                <Form.Item label="Pihak II (Penerima BMN)" name='pihak2_user_id'>
                    <Select options={users} disabled={currentRole === 2} />
                ...
                <Form.Item label="Daftar Barang" name="barangs">
                    <Select  mode='multiple' options={barangs} loading={barangLoading} optionFilterProp='label' />
                </Form.Item>
